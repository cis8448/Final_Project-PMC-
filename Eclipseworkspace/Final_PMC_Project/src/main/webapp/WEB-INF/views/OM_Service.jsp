...
<body>
	...
	<section>
		<div class="Search">
			<input type="text" placeholder="회원 아이디  OR 회원 이름" id="SearchArea"
				value="">
		...
		<table border="1" bordercolor="#3D3D3D" width="1200" height="100"
			...
			<tr bgcolor="blue" align="center">
				<p>
				<td colspan="6" span style="color: white">고객센터 목록</td>
				</p>
			</tr>
			<tr align="center" bgcolor="skyblue">
				<td>점주 아이디</td>
				<td style="width: 30%;">고객센터 제목</td>
			...
			<tbody id="resultSearch" align="center">
				<c:forEach var="no" items="${nList}">
					<tr align="center">
						...
						<td>${no.no_title}</td>
						<td><a href="./OM_ServiceInfo?no_num=${no.no_num}"
   ...
   $.ajax({
      ...
      success: function(data){
         ...
         for(var i=0;i<data.length;i++){
            result += '<td id="Search">'+data[i].no_m_id+'</td>'
            result += '<td>'+data[i].no_p_id+'</td>'
            result += '<td>'+data[i].no_title+'</td>'
            result += '<td>'+data[i].no_content+'</td>'
            result += '<td><a href="./OM_ServiceInfo?no_num='+data[i].no_num+'" class="MemberInfo">상세보기</a></td>'
      }      
